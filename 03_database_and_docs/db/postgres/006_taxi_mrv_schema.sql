-- ZENOV Taxi MRV Schema V1
-- Sprint 2 target: Taxi CSV -> Packet -> Evidence -> MRV -> Green Point.
...
CREATE TABLE IF NOT EXISTS taxi_daily_operations (
  ...
  factor_config_version TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS taxi_mrv_results (
  taxi_mrv_id TEXT PRIMARY KEY,
  operation_id TEXT REFERENCES taxi_daily_operations(operation_id),
  packet_id TEXT REFERENCES trust_packets(packet_id),
  evidence_id TEXT REFERENCES digital_evidence(evidence_id),
  vehicle_id TEXT NOT NULL,
  operation_date DATE NOT NULL,
  distance_km NUMERIC(12,3) NOT NULL,
  energy_consumed_kwh NUMERIC(12,3) NOT NULL,
  baseline_co2e_kg NUMERIC(14,6) NOT NULL,
  ev_co2e_kg NUMERIC(14,6) NOT NULL,
  reduction_co2e_kg NUMERIC(14,6) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_taxi_mrv_results_evidence_id
  ON taxi_mrv_results(evidence_id);
